...
class SequenceGenerator(Sequence):
    ...
    """
    def __init__(self, text, seq_length, mapping,
    ...
    def _generate_X(self, list_index):
        ...
        """
        # Initialization
        X = np.empty((self.batch_size, self.seq_length))

        # Generate data
        for i, ID in enumerate(list_index):
            char_seq = self.text[ID:ID+self.seq_length]
            encoded_seq = encode_sequence(self.mapping, char_seq)
            X[i] = encoded_seq
    ...
    def _generate_y(self, list_index):
        ...
        for i, ID in enumerate(list_index):
            ...
            encoded_char = encode_sequence(self.mapping, next_char)[0]
            y[i][encoded_char] = 1
